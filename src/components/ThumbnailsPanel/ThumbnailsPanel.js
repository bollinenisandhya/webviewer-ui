...
const ThumbnailsPanel = () => {
  ...
  const onPanelResize = ({ bounds }) => {
    ...
    setNumberOfColumns(Math.min(MAX_THUMBNAILS, Math.max(1, Math.floor(bounds.width / thumbnailSize))));
  };

  const updateNumberOfColumns = () => {
    setNumberOfColumns(Math.min(MAX_THUMBNAILS, Math.max(1, Math.floor(width / thumbnailSize))));
  };
